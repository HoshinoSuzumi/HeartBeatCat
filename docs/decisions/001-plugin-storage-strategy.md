...
- 开发者通过 `BRCAT_TARGET` 环境变量指向主项目，Vite 插件自动同步产物到 dev-plugins
...
- **开发体验**：`BRCAT_TARGET` + 自动同步让插件开发者无需手动复制文件
...
### 代价
- 需要开发者正确设置 `BRCAT_TARGET` 环境变量

### 后续
- 需在 `@brcat/vite-plugin` 中实现 `BRCAT_TARGET` 检测和自动同步逻辑
