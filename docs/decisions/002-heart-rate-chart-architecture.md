...
3. **数据采集**：Store 通过轮询 `useBrcatStore().current_heart_rate` 自动采集，不依赖页面挂载。
...
- **独立 Store**：与 `useBrcatStore`（设备连接管理）职责分离，便于插件直接引用而不引入设备管理的副作用。
...
| 数据采集放在 `useBrcatStore` 中 | 职责混乱，插件获取历史数据时被迫依赖设备管理逻辑 |
