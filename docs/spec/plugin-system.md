...
- **构建工具自动化**：插件开发者只需编写 `brcat.config.ts`，manifest 由构建工具生成
...
| `widget.window.defaultWidth` | `number` | 是 | 默认宽度 (px) |
| `widget.window.defaultHeight` | `number` | 是 | 默认高度 (px) |
| `widget.window.minWidth` | `number` | 是 | 最小宽度 (px) |
| `widget.window.minHeight` | `number` | 是 | 最小高度 (px) |
| `widget.window.resizable` | `boolean` | 是 | 是否允许调整大小 |
...
- 若 `widget` 存在，`widget.entry`、`widget.window`、`window.defaultWidth`、`window.defaultHeight`、`window.minWidth`、`window.minHeight` 均必填
...
`.brcp` 为 ZIP 格式压缩包，内部结构同上。安装时解压验证。
...
- **拖拽安装**：将 `.brcp` 文件拖入主窗口
- **按钮安装**：点击「安装插件」按钮，通过文件选择器选择 `.brcp` 文件
...
### 9.1 项目结构（brcat-widget-example）

```
my-plugin/
├── brcat.config.ts           ← 插件专属配置
...
```ts
import { defineConfig } from '@brcat/vite-plugin'
...
### 9.3 @brcat/vite-plugin 行为

1. 读取 `brcat.config.ts` 和 `package.json`
2. 检测 `widget/`、`streaming/` 目录的存在性 → 决定构建哪些入口
3. 为每个入口配置独立的 Vite 构建
4. `writeBundle` 阶段自动生成 `hbcat-manifest.json`
5. 输出 `dist/{plugin-id}/` 并打包为 `dist/{plugin-id}.brcp`
6. 若设置了 `BRCAT_TARGET` 环境变量，dev 模式下自动同步产物到目标项目的 `dev-plugins/`
...
| P2 — 构建工具链 | `@brcat/vite-plugin`：多入口构建、自动生成 manifest、打包 `.brcp` | npm 包 |
...
| P6 — 示例改造 | `brcat-widget-example` 改为新格式参考模板 | 示例项目 |
